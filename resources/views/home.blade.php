...
<body class="bg-white dark:bg-slate-950 font-sans text-slate-900 dark:text-slate-100 antialiased transition-colors">
    <div class="min-h-screen">
        <!-- Navigation -->
        <nav x-data="{ open: false }" class="border-b border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 sticky top-0 z-50 transition-colors">
                ...
                <div class="hidden items-center gap-3 sm:flex">
                    @auth
                        <div class="flex items-center gap-2 rounded-full bg-slate-50 dark:bg-slate-800 px-2 py-1 ring-1 ring-slate-200 dark:ring-slate-700">
                            <a href="{{ route('home') }}" class="{{ request()->routeIs('home') ? 'bg-gradient-primary text-white shadow-sm' : 'text-slate-600 dark:text-slate-400 hover:bg-white dark:hover:bg-slate-700 hover:text-slate-900 dark:hover:text-slate-100' }} rounded-full px-4 py-2 text-sm font-semibold transition">Home</a>
                            <a href="{{ route('my-rentals') }}" class="{{ request()->routeIs('my-rentals') ? 'bg-gradient-primary text-white shadow-sm' : 'text-slate-600 dark:text-slate-400 hover:bg-white dark:hover:bg-slate-700 hover:text-slate-900 dark:hover:text-slate-100' }} rounded-full px-4 py-2 text-sm font-semibold transition">My Rentals</a>
                        ...
                        <x-dark-mode-toggle />
                        <a href="{{ route('profile.show') }}" class="inline-flex group">
                            <x-profile-avatar :user="Auth::user()" />
                        </a>
                    @else
                        <x-dark-mode-toggle />
                        <a href="{{ route('login') }}" class="text-sm font-medium text-slate-600 dark:text-slate-400 hover:text-slate-900 dark:hover:text-slate-100">Sign In</a>
                        <a href="{{ route('register') }}" class="rounded-lg bg-gradient-primary px-4 py-2 text-sm font-semibold text-white hover:shadow-lg hover:shadow-primary-500/50 transition-all">Sign Up</a>
                    @endauth
                </div>
                ...
                <div class="space-y-2 text-sm">
                    @auth
                        <div class="space-y-2 rounded-2xl bg-slate-50 dark:bg-slate-800 p-2 ring-1 ring-slate-200 dark:ring-slate-700">
                            <a href="{{ route('home') }}" class="{{ request()->routeIs('home') ? 'bg-gradient-primary text-white' : 'text-slate-600 dark:text-slate-400 hover:bg-white dark:hover:bg-slate-700 hover:text-slate-900 dark:hover:text-slate-100' }} block rounded-xl px-4 py-3 font-semibold transition">Home</a>
                            <a href="{{ route('my-rentals') }}" class="{{ request()->routeIs('my-rentals') ? 'bg-gradient-primary text-white' : 'text-slate-600 dark:text-slate-400 hover:bg-white dark:hover:bg-slate-700 hover:text-slate-900 dark:hover:text-slate-100' }} block rounded-xl px-4 py-3 font-semibold transition">My Rentals</a>
                            <a href="{{ route('my-listings') }}" class="{{ request()->routeIs('my-listings') || request()->routeIs('add-item') || request()->routeIs('item.view') ? 'bg-gradient-primary text-white' : 'text-slate-600 dark:text-slate-400 hover:bg-white dark:hover:bg-slate-700 hover:text-slate-900 dark:hover:text-slate-100' }} block rounded-xl px-4 py-3 font-semibold transition">My Listings</a>
                        </div>
                    ...
                    @else
                        <a href="{{ route('login') }}" class="block text-slate-600 dark:text-slate-400 hover:text-slate-900 dark:hover:text-slate-100 py-2 font-medium">Sign In</a>
                        <a href="{{ route('register') }}" class="block font-semibold bg-gradient-primary text-white rounded-lg px-4 py-2 text-center">Sign Up</a>
                    @endauth
                </div>
            </div>
        </nav>
        ...
        <section class="relative overflow-hidden bg-gradient-primary dark:bg-gradient-to-br dark:from-primary-900 dark:to-accent-900 text-white">
            ...
            <div class="relative mx-auto max-w-7xl px-4 py-16 sm:px-6 lg:px-8">
                <div class="mx-auto max-w-4xl text-center">
                    <h1 class="text-5xl sm:text-7xl font-black tracking-tighter leading-tight">
                    ...
                    <p class="mx-auto mt-6 max-w-2xl text-lg sm:text-xl text-blue-100 dark:text-accent-200">Rent and lend items within the University of Mindanao campus community. Connect, share, and save together.</p>

                    <!-- Enhanced Search Bar with Gradient Button -->
                    <form class="mx-auto mt-12 flex max-w-2xl items-center gap-3 rounded-2xl bg-white/95 dark:bg-slate-900/95 p-2 shadow-2xl backdrop-blur-xl ring-1 ring-white/20 dark:ring-slate-700/50">
                        <div class="flex min-w-0 flex-1 items-center gap-3 px-4">
                            ...
                            <input
                                type="text"
                                placeholder="Search for electronics, books, gadgets..."
                                class="w-full min-w-0 border-0 bg-transparent text-lg text-slate-900 dark:text-slate-100 placeholder:text-slate-400 dark:placeholder:text-slate-500 focus:outline-none focus:ring-0"
                            >
                        </div>
                        <button
                            type="button"
                            class="shrink-0 h-12 rounded-xl bg-gradient-primary px-8 text-lg font-semibold text-white hover:shadow-lg hover:shadow-primary-500/50 transition-all duration-300 hover:-translate-y-0.5"
                        >
                            Search
                        </button>
                    </form>
        ...
        <!-- Categories Section -->
        <section class="border-b border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900">
            <div class="mx-auto max-w-7xl px-4 py-12 sm:px-6 lg:px-8">
                <h2 class="mb-6 text-2xl font-bold text-slate-900 dark:text-slate-50">Browse Categories</h2>
                <div class="mx-auto flex max-w-5xl flex-wrap items-center justify-center gap-2 rounded-2xl bg-slate-50 dark:bg-slate-800 p-3 shadow-sm ring-1 ring-slate-200 dark:ring-slate-700">
                    <a
                        href="{{ route('home') }}"
                        class="{{ $selectedCategory === null ? 'bg-white dark:bg-slate-700 text-slate-900 dark:text-slate-50 shadow-sm ring-1 ring-slate-200 dark:ring-slate-600' : 'text-slate-500 dark:text-slate-400 hover:bg-white dark:hover:bg-slate-700 hover:text-slate-900 dark:hover:text-slate-100' }} inline-flex items-center gap-2 rounded-xl px-5 py-3 text-sm font-semibold transition"
                    >
                    ...
                    @foreach ($categories as $category)
                        <a
                            ...
                            class="{{ optional($selectedCategory)->is($category) ? 'bg-white dark:bg-slate-700 text-slate-900 dark:text-slate-50 shadow-sm ring-1 ring-slate-200 dark:ring-slate-600' : 'text-slate-500 dark:text-slate-400 hover:bg-white dark:hover:bg-slate-700 hover:text-slate-900 dark:hover:text-slate-100' }} inline-flex items-center gap-2 rounded-xl px-5 py-3 text-sm font-semibold transition"
                        >
        ...
        <!-- Items Grid -->
        <main class="bg-white dark:bg-slate-950 mx-auto max-w-7xl px-4 py-12 sm:px-6 lg:px-8">
            <div class="mb-8 flex items-center justify-between">
                <div>
                    <h2 class="text-2xl font-bold text-slate-900 dark:text-slate-50">All Available Items</h2>
                    <p class="mt-1 text-slate-500 dark:text-slate-400">Explore rentals shared by your campus community.</p>
                </div>
                <span class="rounded-full bg-slate-100 dark:bg-slate-800 px-4 py-2 text-sm font-semibold text-slate-600 dark:text-slate-300 ring-1 ring-slate-200 dark:ring-slate-700">
                    {{ $featuredItems->count() }} items
                </span>
            </div>

            @if ($featuredItems->isEmpty())
                <div class="rounded-2xl border-2 border-dashed border-slate-300 dark:border-slate-600 bg-slate-50 dark:bg-slate-800 px-6 py-20 text-center">
            ...
            @else
                <div class="grid grid-cols-2 gap-4 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5">
                    @foreach ($featuredItems as $item)
                        <article class="group overflow-hidden rounded-2xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 shadow-sm dark:shadow-slate-900/50 transition hover:-translate-y-1 hover:shadow-lg dark:hover:shadow-slate-900/70">
                            <div class="relative aspect-[4/3] overflow-hidden bg-slate-100 dark:bg-slate-800">
                                @if ($item->image_path)
                                    <img src="{{ asset('storage/' . $item->image_path) }}" alt="{{ $item->name }}" class="h-full w-full object-cover group-hover:scale-110 transition duration-300">
                                @else
                            ...
                            <div class="space-y-3 p-4">
                                ...
                                <div class="flex items-end justify-between pt-2 border-t border-slate-200 dark:border-slate-700">
                                    <div>
                                        <div class="text-lg font-bold bg-gradient-primary bg-clip-text text-transparent">₱{{ number_format($item->price, 2) }}</div>
                                ...
                                @auth
                                    <a href="{{ route('item.view', $item->id) }}" class="block rounded-lg bg-gradient-primary hover:shadow-lg hover:shadow-primary-500/50 px-3 py-2.5 text-center text-xs font-semibold text-white transition-all duration-300 hover:-translate-y-0.5">
                                        View Details
                                    </a>
                                @else
                                    <a href="{{ route('login') }}" class="block rounded-lg bg-slate-900 dark:bg-slate-800 hover:bg-slate-800 dark:hover:bg-slate-700 px-3 py-2.5 text-center text-xs font-semibold text-white transition-all">
                                        Sign In
                                    </a>
                                @endauth
                            </div>
